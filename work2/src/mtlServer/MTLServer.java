...
public class MTLServer extends ClinicInterfacePOA implements ClinicInterface{
	...
	public int createDRecord(String managerID,String firstName,String lastName,String address,String phone,String specialization,String location){
		...
		Log.write(managerID + " create doctor record " + firstName + " " + lastName  + " " + address 
				 + " " + phone + " " + specialization + " " + location + " " + d.recordID, "MTLServer.txt");
		return 1;
	}
	...
	public int createNRecord (String managerID,String firstName,String lastName,String designation,String status,String statusDate){
		...
		Log.write(managerID + " create nurse record " + firstName + " " + lastName  + " " + designation 
				 + " " + status + " " + statusDate + " " + n.recordID, "MTLServer.txt");
		return 1;
	}
	...
	public int verifyManagerID (String id){
		...
		Log.write(id + " connect to MTL ", "MTLServer.txt");
		if(managerAllowID.contains(id)){
			Log.write(id + " accept ", "MTLServer.txt");
			nowID = id;
			return 1;
		}else{
			Log.write(id + " reject ", "MTLServer.txt");
			return 0;
		}
...
public class ThreadS implements Runnable{
		...
		public void run(){
			try{
						...
						aSocket.send(reply);
						Log.write(" reply a Count request ", "MTLServer.txt");
			}catch(Exception e){
				e.printStackTrace();
			}
		}
...
public int editRecord (String managerID,String ID,String fieldName,String newValue){
	...
	Log.write(managerID + " edit record " + ID + " " + fieldName  + " " + newValue , "MTLServer.txt");
	Record r = searchID(ID);
	if(r == null){
		Log.write(managerID + " edit record fail", "MTLServer.txt");
	...
	synchronized(r){
		...
		Log.write(managerID + " edit record success", "MTLServer.txt");
	}
	return 1;
}
...
public int transferRecord (String managerID,String ID,String remoteName){
	...
	try{
			if(remoteName.equals("MTL")){
				...
				Log.write("transfer to MTL ", "MTLServer.txt");
				...
				Log.write("transfer to LVL ", "MTLServer.txt");
				...
				Log.write("transfer to DDO ", "MTLServer.txt");
				Log.write("transfer from DDO ", "DDOServer.txt");
			}else{
				System.out.println("remoteName error");
				Log.write("wrong remoteName", "MTLServer.txt");
				return 0;
			}
			if(ID.substring(0, 2).equals("DR")){
				...
				if(success == 1){
					...
					Log.write(managerID + " create doctor record success", "MTLServer.txt");
					synchronized(recordTemp){
					...
					Log.write(managerID + " create doctor record fail", "MTLServer.txt");
				}
			}else{
				...
				if(success == 1){
					System.out.println("create successfully");
					Log.write(managerID + " create nurse record success", "MTLServer.txt");
					synchronized(recordTemp){
						recordTemp.remove(ID);
						}
					n.numdecre();
					...
					System.out.println("create error");
					Log.write(managerID + " create nurse record fail", "MTLServer.txt");
				}
		        }
