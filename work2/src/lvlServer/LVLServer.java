...
public class LVLServer extends ClinicInterfacePOA implements ClinicInterface{
	...
	HashMap <String,Record> recordID = new HashMap <String,Record>();
	...
	public LVLServer() throws RemoteException{
	...
	public int createDRecord(String managerID,String firstName,String lastName,String address,String phone,String specialization,String location){
		...
		Log.write(managerID + " create doctor record " + firstName + " " + lastName  + " " + address 
				 + " " + phone + " " + specialization + " " + location + " " + d.recordID, "LVLServer.txt");
		return 1;
	}
	...
	public int createNRecord (String managerID,String firstName,String lastName,String designation,String status,String statusDate){
		...
		Log.write(managerID + " create nurse record " + firstName + " " + lastName  + " " + designation 
				 + " " + status + " " + statusDate + " " + n.recordID, "LVLServer.txt");
		return 1;
	}
	...
	public int verifyManagerID (String id){
		...
		Log.write(id + " connect to LVL ", "LVLServer.txt");
		if(managerAllowID.contains(id)){
			Log.write(id + " accept ", "LVLServer.txt");
			nowID = id;
			return 1;
		}else{
			Log.write(id + " reject ", "LVLServer.txt");
			return 0;
		}
...
public class ThreadS implements Runnable{
		...
		public void run(){
			try{
						...
						aSocket.send(reply);
						Log.write(" reply a Count request ", "LVLServer.txt");
			}catch(Exception e){
				e.printStackTrace();
			}
		}
...
public int editRecord (String managerID,String ID,String fieldName,String newValue){
	...
	Log.write(managerID + " edit record " + ID + " " + fieldName  + " " + newValue , "LVLServer.txt");
	Record r = searchID(ID);
	if(r == null){
		Log.write(managerID + " edit record fail", "LVLServer.txt");
	...
	synchronized(r){
		...
		Log.write(managerID + " edit record success", "LVLServer.txt");
	}
	return 1;
}
...
public int transferRecord (String managerID,String ID,String remoteName){
	...
	try{
			if(remoteName.equals("MTL")){
				...
				Log.write("transfer to MTL ", "LVLServer.txt");
				...
				Log.write("transfer to LVL ", "LVLServer.txt");
				...
				Log.write("transfer to DDO ", "LVLServer.txt");
				Log.write("transfer from DDO ", "DDOServer.txt");
			}else{
				System.out.println("remoteName error");
				Log.write("wrong remoteName", "LVLServer.txt");
				return 0;
			}
			if(ID.substring(0, 2).equals("DR")){
				...
				if(success == 1){
					...
					Log.write(managerID + " create doctor record success", "LVLServer.txt");
					synchronized(recordTemp){
					...
					Log.write(managerID + " create doctor record fail", "LVLServer.txt");
				}
			}else{
				...
				if(success == 1){
					System.out.println("create successfully");
					Log.write(managerID + " create nurse record success", "LVLServer.txt");
					synchronized(recordTemp){
						recordTemp.remove(ID);
						}
					n.numdecre();
					...
					System.out.println("create error");
					Log.write(managerID + " create nurse record fail", "LVLServer.txt");
				}
		        }
	...
	public static void main(String[] args) {
		try{
			LVLServer server = new LVLServer();
